...
let do_random st sz c dumb =
  ...
  let rec f c dumb i =
    if i = sz then (c, dumb)
      ...
      let (c, dumb) = 
        ...
        | `Commit ->
            let Cursor(_, _, context), i, _ = Cursor_storage.commit_cursor c in
            let v = Storage.load_node context i Not_Extender in
            add_op op;
  ...
  (List.rev !rev_ops, 
   let Cursor(tr, n, context) = c in
   _Cursor (tr, Storage.load_node_fully context n, context))
