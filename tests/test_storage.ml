...
let parse_test context n =
  ...
  try
    let n' = View (Context_storage.parse_cell context.Context.storage @@ from_Some @@ index n) in
    let n' = Context_storage.load_node_fully context n' in
    if n <> n' then begin
...
let random_write_parse st =
  ...
  | 0 (* leaf *) ->
      ...
      let n = new_leaf v in
      let n, _i, _h = Context_storage.commit_node context n in
      parse_test context n

  | 1 (* bud *) ->
      if RS.bool st then
        let n = new_bud None in
        let n, _, _ = Context_storage.commit_node context n in
        parse_test context n
      else
        let size = RS.int st 64 in
        let v = Value.of_string @@ random_string st size in
  ...
  | 2 (* internal *) ->
      ...
      let n1, _, _ =
        Context_storage.commit_node context @@
